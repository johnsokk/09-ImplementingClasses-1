...
#
# DONE (continued): Then implement a class called   Point that has NO METHODSj# zqa   DONE (continued)  yet, just the lines that start the definition of any class:
#
...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        self.x = x
        self.y = y
        self.distance = self.distance + self.get_distance_from(Point(tempx, tempy))
    ...
    def closer_to(self, p2, p3):
        dist12 = self.get_distance_from(p2)
        dist13 = self.get_distance_from(p3)
        if dist12 > dist13:
            return p3
        return p2

    def halfway_to(self, other):
        xhalf = (self.x + other.x)/2
        yhalf = (self.y + other.y)/2
        return Point(xhalf, yhalf)
